((cssText, artDataUrl, themeConfig, decorationAssets) => {
  const STATE_KEY = "__CODEX_DREAM_SKIN_STATE__";
  const DISABLED_KEY = "__CODEX_DREAM_SKIN_DISABLED__";
  const STYLE_ID = "codex-dream-skin-style";
  const CHROME_ID = "codex-dream-skin-chrome";
  const DECORATION_ID = "codex-dream-skin-decorations";
  ...
  const COPY = THEME.copy && typeof THEME.copy === "object" ? THEME.copy : {};
  const DECORATIONS = THEME.decorations && typeof THEME.decorations === "object"
    ? THEME.decorations : {};
  const MOTION = THEME.motion && typeof THEME.motion === "object" ? THEME.motion : {};
  const DECORATION_ASSETS = decorationAssets && typeof decorationAssets === "object"
    ? decorationAssets : {};
  const DECORATION_ATTRS = [
    "data-dream-decoration-profile",
    "data-dream-motion-profile",
    "data-dream-route",
    "data-dream-motion-paused",
  ];
  ...
  const THEME_VARIABLES = [
    ...
    "--dream-skin-project-label", "--dream-home-intensity", "--dream-task-intensity",
  ];
  const installToken = {};
  ...
  const applyTheme = (root, shell) => {
    ...
    setStyleProperty(root, "--dream-home-intensity", String(MOTION.homeIntensity ?? 0));
    setStyleProperty(root, "--dream-task-intensity", String(MOTION.taskIntensity ?? 0));
  };
  ...
  const appendDecoration = (parent, tagName, className, text = "") => {
  ...
  const buildDecorationLayer = (layer) => {
    appendDecoration(layer, "div", "dream-decoration-sheen");
    appendDecoration(layer, "div", "dream-decoration-flourish", "— ❧ —");
    appendDecoration(layer, "div", "dream-decoration-seal", "V");
    appendDecoration(layer, "div", "dream-decoration-corner", "❦");
    appendDecoration(layer, "div", "dream-decoration-pen", "✒");

    const copy = appendDecoration(layer, "div", "dream-decoration-copy");
    appendDecoration(copy, "small", "dream-decoration-eyebrow", COPY.eyebrow || "");
    appendDecoration(copy, "strong", "dream-decoration-title", COPY.title || "");
    appendDecoration(copy, "span", "dream-decoration-tagline", COPY.tagline || THEME.tagline || "");

    const portrait = Object.values(DECORATION_ASSETS).find((asset) =>
      asset && (asset.kind === "stamp" || asset.kind === "portrait") &&
      typeof asset.url === "string");
    if (portrait) {
      const image = appendDecoration(layer, "img", "dream-decoration-portrait");
      image.src = portrait.url;
      image.alt = "";
      image.setAttribute("draggable", "false");
    }

    const petals = appendDecoration(layer, "div", "dream-decoration-petals");
    for (let index = 0; index < 8; index += 1) {
      const petal = appendDecoration(petals, "i", "dream-decoration-petal", "✦");
      petal.style.setProperty("--dream-petal-index", String(index));
    }
  };

  const ensureDecorations = (home) => {
    const root = document.documentElement;
    if (!root) return;
    const profile = DECORATIONS.profile || "none";
    const motionProfile = MOTION.profile || "none";
    if (profile === "none") {
      document.getElementById(DECORATION_ID)?.remove();
      for (const name of DECORATION_ATTRS) root.removeAttribute(name);
      return;
    }

    setAttribute(root, "data-dream-decoration-profile", profile);
    setAttribute(root, "data-dream-motion-profile", motionProfile);
    setAttribute(root, "data-dream-route", home ? "home" : "task");
    let layer = document.getElementById(DECORATION_ID);
    if (!layer || layer.parentElement !== document.body) {
      layer?.remove();
      layer = document.createElement("div");
      layer.id = DECORATION_ID;
      layer.className = "dream-skin-decorations";
      layer.setAttribute("aria-hidden", "true");
      buildDecorationLayer(layer);
      document.body?.appendChild(layer);
    }
    layer.classList.toggle("dream-skin-decorations-home", Boolean(home));
    layer.classList.toggle("dream-skin-decorations-task", !home);
  };
  ...
  const syncRouteState = (shell, { layout = false } = {}) => {
    ...
    ensureDecorations(Boolean(home));
    syncMotionState();
  ...
  const cleanup = () => {
    ...
    document.getElementById(DECORATION_ID)?.remove();
    for (const name of DECORATION_ATTRS) document.documentElement?.removeAttribute(name);
  ...
  const syncMotionState = () => {
    const root = document.documentElement;
    if (!root || (DECORATIONS.profile || "none") === "none") return;
    const paused = Boolean(document.hidden || motionQuery?.matches || window[DISABLED_KEY]);
    setAttribute(root, "data-dream-motion-paused", paused ? "true" : "false");
  };
  const visibilityHandler = () => syncMotionState();
  try {
    motionQuery = window.matchMedia("(prefers-reduced-motion: reduce)");
    motionHandler = () => syncMotionState();
  } catch {}
  ...
  __DREAM_SKIN_DECORATIONS_JSON__
)
