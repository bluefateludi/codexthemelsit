...
try {
  ...
  const initialManifest = JSON.parse(await runStage(source, stage));
  assert.equal(initialManifest.image, "background-a.png");
  assert.deepEqual(initialManifest.files, ["background-a.png"]);
  ...
  const multiSource = path.join(tempRoot, "themes", "custom-multi");
  const multiStage = path.join(tempRoot, "multi-stage");
  await fs.mkdir(multiSource, { recursive: true });
  await fs.mkdir(multiStage);
  await fs.copyFile(fixtureAsset, path.join(multiSource, "background.png"));
  await fs.copyFile(fixtureAsset, path.join(multiSource, "stamp.png"));
  await fs.writeFile(
  ...
  const manifest = JSON.parse(await runStage(multiSource, multiStage));
  assert.deepEqual(manifest.files, ["background.png", "stamp.png"]);
  assert.deepEqual(
    await fs.readFile(path.join(multiStage, "stamp.png")),
    await fs.readFile(path.join(multiSource, "stamp.png")),
  );

  const linkedDecoration = path.join(tempRoot, "linked-decoration");
  const linkedStage = path.join(tempRoot, "linked-stage");
  await fs.mkdir(linkedDecoration);
  await fs.mkdir(linkedStage);
  await fs.copyFile(fixtureAsset, path.join(linkedDecoration, "background.png"));
  await fs.symlink(outside, path.join(linkedDecoration, "stamp.png"));
  await fs.writeFile(
    path.join(linkedDecoration, "theme.json"),
    `${JSON.stringify({
      schemaVersion: 2,
      id: "linked-decoration",
      image: "background.png",
      decorations: {
        profile: "postal-balanced",
        assets: [{ id: "stamp", file: "stamp.png", kind: "stamp" }],
  ...
  await assert.rejects(
    runStage(linkedDecoration, linkedStage),
    /symbolic link/,
  );

  console.log("PASS: theme staging snapshots a matched, contained multi-asset pack.");
} finally {
