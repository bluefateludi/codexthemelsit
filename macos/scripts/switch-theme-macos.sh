...
STAGE_MANIFEST="$("$NODE" "$SCRIPT_DIR/stage-theme.mjs" "$SRC" "$stage")" \
  || fail "Theme pack changed or failed staging: $THEME_ID"
THEME_IMAGE="$("$NODE" -e '
  const value = JSON.parse(process.argv[1]);
  if (typeof value.image !== "string" || !Array.isArray(value.files)) process.exit(1);
  process.stdout.write(value.image);
' "$STAGE_MANIFEST")" || fail "Theme staging returned an invalid manifest: $THEME_ID"
...
"$NODE" -e '
  ...
  const [dir, manifestJson] = process.argv.slice(1);
  const manifest = JSON.parse(manifestJson);
  const keep = new Set(["theme.json", ...manifest.files]);
