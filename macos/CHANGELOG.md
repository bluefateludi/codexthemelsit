...
- 可选主题 schema v2：在原有背景图之上支持首页文案、印章/肖像/纹理等装饰素材、路由感知装饰密度和动效强度。
- 新增 `postal-balanced` 邮政工坊视觉档案与 `expressive` 动效档案，包含象牙纸、藏蓝、古金、火漆、羽笔、流光与花瓣，同时保留原生 Codex 控件和交互。
- `write-theme.mjs` 与图片加载器新增精品主题参数，可创建包含多个本地素材的个人主题包。

### 改进

- 多素材主题在切换前作为一个完整快照暂存，校验真实路径、格式、大小和清单后再原子发布；旧素材不会残留到新主题。
- 首页使用完整动效，任务与工具路由自动降低装饰强度并隐藏大幅肖像；页面隐藏、系统“减少动态效果”或暂停皮肤时会停止动画。
- schema v1 主题和现有的一张图导入流程保持兼容，无需迁移。

### 说明

- 个人版权图片只保存在用户自己的 Dream Skin 主题库中，不进入安装包或仓库；公开分发主题包前需自行确认素材权利。
