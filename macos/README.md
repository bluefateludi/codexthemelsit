...
## Premium multi-asset themes

Schema v2 is optional and backward compatible. It adds a single owned,
non-interactive decoration layer above the existing full-window background.
Home routes can use expressive motion; task and utility routes automatically
reduce the composition so code, diffs, search, and the composer remain usable.
System Reduce Motion and hidden windows receive a complete static fallback.

Create a Postal Atelier theme from a local image:

```bash
~/.codex/codex-dream-skin-studio/scripts/load-image-theme-macos.sh \
  --file "/path/to/background.jpg" \
  --name "Violet · Postal Atelier" \
  --appearance light --focus-x 0.82 --focus-y 0.50 \
  --safe-area left --task-mode ambient \
...
An optional PNG/JPEG/WebP decoration can be copied into the private theme pack:

```bash
  --decoration-file "/path/to/violet-portrait.png" \
  --decoration "portrait:stamp:violet-portrait.png"
```

Use only artwork you are allowed to use. Personal assets remain under
`~/Library/Application Support/CodexDreamSkinStudio/themes/`; the engine does
not upload or bundle them. Switching to any schema v1 preset removes the
Postal decoration layer. To return completely to the official appearance:
